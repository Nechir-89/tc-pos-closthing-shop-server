...
export const get_stocks_service = async () => {
  ...
  try {
    const respond = await db.any(
      `SELECT * FROM ${process.env.DB_SCHEMA}.stocking`
    );
...
export const get_stock_by_stocking_id_service = async (
  ...
  try {
    const respond = await db.one(
      `SELECT * FROM ${process.env.DB_SCHEMA}.stocking WHERE stocking_id=$<stocking_id>`,
      { stocking_id }
    );
    console.log(`Passed: all stocks found`);
    return respond;
...
export const delete_stock_service = async (stocking_id: number) => {
  ...
  try {
    ...
    console.log(
      `Failed: deleting stock with stocking id ${stocking_id} ==> ${error}`
    );
    return { error: `DB error` };
  }
};
...
export const delete_stock_service_2 = async (
  ...
  try {
    ...
    const respond = await db.none(query, { stocking_id });
    console.log(
      `Passed: Updating item state and deleting stock, and stock state for stocking id ${stocking_id}`
    );
    return respond;
  } catch (error) {
    console.log(
      `Failed: Updating item state and deleting stock, and stock state for stocking id ${stocking_id} ==> ${error}`
    );
    return { error: `DB error` };
  }
};

export const update_stock_expire_date_service = async (
  stocking_id: number,
  expire_date: string
) => {
...
export const update_stock_barcodes_service = async (
  stocking_id: number,
